...
def plot_correlation(corr_name, column_name, plot_name, dpi):
...
def shap_beeswarm_plot(shap_values_mol, color_plot_shap, plot_name, dpi):
...
def cluster_scatter_plot(s2d_values, color_plot_dbscan, plot_name, feature, column_feature, column_feature_stan, size_plt, alpha_plt, minx, maxx, dpi):    
...
# # xgb_model = xgb.XGBRegressor(n_estimators=par[0],max_depth=par[1],reg_lambda=par[2],reg_alpha=par[3],learning_rate=par[4],objective=par[5], min_child_weight=par[6],gamma=par[7])
# # xgb_model.fit(X_train,y_train)

mse_train = mean_squared_error(y_train, linreg_model.predict(X_train))
print("MSE Train Set XGB Regression: %.4f" % mse_train)
print("RMSE Train Set XGB Regression: %.4f" % np.sqrt(mse_train)) 
mae_train = mean_absolute_error(y_train, linreg_model.predict(X_train))
print("MAE Train Set XGB Regression: %.4f" % mae_train)  
mse = mean_squared_error(y_test, linreg_model.predict(X_test))
...
results_linreg = model_selection.cross_val_score(linreg_model, X_train, y_train, cv=kfold)
print("Accuracy: Final mean:%.3f%%, Final standard deviation:(%.3f%%)" % (results_linreg.mean()*100.0, results_linreg.std()*100.0))
print('Accuracies from each of the number folds using kfold:',results_linreg)
print("Variance of kfold accuracies:",results_linreg.var())

filename = 'Results_' + model + '_total'

outfile = open(filename, "w")

outfile.write("MSE Train Set Linear Regression: " + str(round(mse_train,3)) + "\n")
outfile.write("RMSE Train Set Linear Regression: " + str(round(np.sqrt(mse_train),3)) + "\n")
outfile.write("MAE Train Set Linear Regression: " + str(round(mae_train,3)) + "\n")
outfile.write("MSE Test Set Linear Regression: " + str(round(mse,3)) + "\n")
outfile.write("RMSE Test Set Linear Regression: " + str(round(np.sqrt(mse),3)) + "\n")
outfile.write("MAE Test Set Linear Regression: " + str(round(mae,3)) + "\n")
outfile.write("Accuracy: Final mean: " + str(round(results_linreg.mean()*100.0,3)) + "%, Final standard deviation: " + str(round(results_linreg.std()*100.0,3)) + "%\n")
outfile.write('Accuracies from each of the number folds using kfold: ' + str(results_linreg) + "\n")
outfile.write("Variance of kfold accuracies:" + str(results_linreg.var()) + "\n")
# outfile.write("\n")

outfile.close()

# explainer = shap.Explainer(linreg_model)
...
plt.axis('square')
plt.legend()
plt.title(label='ML vs DFT adsorption energy for the MLR model', fontsize=13)
plt.tight_layout()
