...
# plot_correlation(correlations_scaled_robust_surf, correlations_scaled_robust_surf, 'correlations_scaled_robust_surf', dpi_qua)
# plot_correlation(correlations_scaled_robust_ads, correlations_scaled_robust_ads, 'correlations_scaled_robust_ads', dpi_qua)
...
name = 'results_lightboost_total_par_1.pkl'
...
#         light_model = LGBMRegressor(n_estimators=500, learning_rate=0.2, max_depth=5, boosting_type='gbdt', num_leaves=(2**5), verbosity=-1, min_data_in_leaf=child, lambda_l2=L2)    
#         light_model.fit(X_train,y_train)
        
#         mse_train = mean_squared_error(y_train, light_model.predict(X_train))
#         print("MSE Train Set XGB Regression: %.4f" % mse_train)    
#         mae_train = mean_absolute_error(y_train, light_model.predict(X_train))
#         # print("MAE Train Set XGB Regression: %.4f" % mae_train)  
#         mse = mean_squared_error(y_test, light_model.predict(X_test))
#         print("MSE Test Set XGB Regression: %.4f" % mse)
#         mae = mean_absolute_error(y_test, light_model.predict(X_test))
#         # print("MAE Test Set XGB Regression: %.4f" % mae)
        
#         # xgb_model.get_xgb_params()
        
#         results_cat = model_selection.cross_val_score(light_model, X_train, y_train, cv=kfold)
...
# name = 'results_lightboost_total_par_2.pkl'
# results.to_pickle(name)
# best_tunning = pd.read_pickle(name)
# # par = [900, 5, 0, 1.2, 0.12, 'reg:squarederror', 6, 0]
light_model = LGBMRegressor(n_estimators=500, learning_rate=0.2, max_depth=5, boosting_type='gbdt', num_leaves=(2**5), verbosity=-1, min_data_in_leaf=2, lambda_l2=0.8)    
light_model.fit(X_train,y_train)


mse_train = mean_squared_error(y_train, light_model.predict(X_train))
print("MSE Train Set LightG Regression: %.4f" % mse_train)
print("RMSE Train Set LightG Regression: %.4f" % np.sqrt(mse_train)) 
mae_train = mean_absolute_error(y_train, light_model.predict(X_train))
print("MAE Train Set LightG Regression: %.4f" % mae_train)  
mse = mean_squared_error(y_test, light_model.predict(X_test))
print("MSE Test Set LightG Regression: %.4f" % mse)
print("RMSE Test Set LightG Regression: %.4f" % np.sqrt(mse))
mae = mean_absolute_error(y_test, light_model.predict(X_test))
print("MAE Test Set LightG Regression: %.4f" % mae)

results_light = model_selection.cross_val_score(light_model, X_train, y_train, cv=kfold)
print("Accuracy: Final mean:%.3f%%, Final standard deviation:(%.3f%%)" % (results_light.mean()*100.0, results_light.std()*100.0))
print('Accuracies from each of the number folds using kfold:',results_light)
print("Variance of kfold accuracies:",results_light.var())

filename = 'Results_' + model + '_total'

outfile = open(filename, "w")

outfile.write("MSE Train Set LightG Regression: " + str(round(mse_train,3)) + "\n")
outfile.write("RMSE Train Set LightG Regression: " + str(round(np.sqrt(mse_train),3)) + "\n")
outfile.write("MAE Train Set LightG Regression: " + str(round(mae_train,3)) + "\n")
outfile.write("MSE Test Set LightG Regression: " + str(round(mse,3)) + "\n")
outfile.write("RMSE Test Set LightG Regression: " + str(round(np.sqrt(mse),3)) + "\n")
outfile.write("MAE Test Set LightG Regression: " + str(round(mae,3)) + "\n")
outfile.write("Accuracy: Final mean: " + str(round(results_light.mean()*100.0,3)) + "%, Final standard deviation: " + str(round(results_light.std()*100.0,3)) + "%\n")
outfile.write('Accuracies from each of the number folds using kfold: ' + str(results_light) + "\n")
outfile.write("Variance of kfold accuracies:" + str(results_light.var()) + "\n")
# outfile.write("\n")
...
shap_beeswarm_plot(shap_values, color_plot, 'overall_' + model + '_total', dpi_qua, 'Summary of SHAP values for the LightGBM model')
...
mask_bias = light_model.predict(X_train)-y_train
mask_bias = np.argwhere((mask_bias>=1) | (mask_bias<=-1))

new_dif_mask_y = np.array(y_train-light_model.predict(X_train))
...
y_pred_train = light_model.predict(X_train)
...
plt.axis('square')
plt.legend()
plt.title(label='ML vs DFT adsorption energy for the LightGBM model', fontsize=13)
plt.tight_layout()
