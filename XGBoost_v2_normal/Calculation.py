...
#     for rat in np.arange(0.06, 0.24, 0.02):##0.3
...
#                 results_xgb = model_selection.cross_val_score(xgb_model, X_train, y_train, cv=kfold)
...
#                 results.append([est, rat, dep, mse_train, mse, results_xgb.mean(), results_xgb.std()*100.0])
...
# name = 'results_xgboost_total_par_1.pkl'
...
#         par = [1300, 7, 0, L2, 0.16, 'reg:squarederror', child, 0]
#         xgb_model = xgb.XGBRegressor(n_estimators=par[0],max_depth=par[1],reg_lambda=par[2],reg_alpha=par[3],learning_rate=par[4],objective=par[5], min_child_weight=par[6],gamma=par[7])
#         xgb_model.fit(X_train,y_train)
...
#         results.append([child, L2, mse_train, mse, results_xgb.mean(), results_xgb.std()*100.0])

# results = pd.DataFrame(results, columns=['min_leaf', 'reg_lambda', 'mse_train', 'mse_test', 'Accuracy_k_folds', 'deviation_k_folds'])
name = 'results_xgboost_total_par_2.pkl'
# results.to_pickle(name)

best_tunning = pd.read_pickle(name)

par = [1300, 7, 0, 0.2, 0.16, 'reg:squarederror', 9, 0]
...
outfile.write("Accuracy: Final mean: " + str(round(results_xgb.mean()*100.0,3)) + "%, Final standard deviation: " + str(round(results_xgb.std()*100.0,3)) + "%\n")
outfile.write('Accuracies from each of the number folds using kfold: ' + str(results_xgb) + "\n")
outfile.write("Variance of kfold accuracies:" + str(results_xgb.var()) + "\n")
# outfile.write("\n")
...
shap_beeswarm_plot(shap_values, color_plot, 'overall_' + model + '_total', dpi_qua, 'Summary of SHAP values for the XGBoost model')
...
mask_bias = xgb_model.predict(X_train)-y_train
mask_bias = np.argwhere((mask_bias>=1) | (mask_bias<=-1))

new_dif_mask_y = np.array(y_train-xgb_model.predict(X_train))
...
plt.axis('square')
plt.legend()
plt.title(label='ML vs DFT adsorption energy for the XGBoost model', fontsize=13)
plt.tight_layout()
